...
from keyboards.default.products_kb import make_product_keyboard
...
@dp.callback_query(F.data == 'add_cart_to', BuyurtmaBerishState.product)
async def product_plus_handler(call: CallbackQuery, state: FSMContext):
    data = await state.get_data()
    quantity = data.get('quantity')
    name = data.get('name')
    ...
    await call.message.answer('Mahsulot bazaga qo\'shiladi')
